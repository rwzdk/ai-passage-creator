...
import com.qc.template.model.vo.HotTopicItemVO;
...
public class HotTopicServiceImpl implements HotTopicService {
    ...
    private static final int REFRESH_BATCH_SHIFT = 3;
    ...
    public HotTopicsVO getHotTopics(boolean refresh) {
        ...
        if (gNewsConfig.getApiKey() == null || gNewsConfig.getApiKey().isBlank()) {
            HotTopicsVO result = fallbackTopics();
            if (refresh && cachedTopics != null) {
                result.setItems(rotateItems(cachedTopics.getItems(), refreshBatchShift(result.getItems().size())));
            }
            cachedTopics = result;
        ...
        try {
            ...
            if (result.getItems() != null && !result.getItems().isEmpty()) {
                if (refresh && cachedTopics != null && sameTopics(cachedTopics.getItems(), result.getItems())) {
                    result.setItems(rotateItems(cachedTopics.getItems(), refreshBatchShift(result.getItems().size())));
                }
                cachedTopics = result;
                cachedAt = System.currentTimeMillis();
    ...
    static List<HotTopicItemVO> rotateItems(List<HotTopicItemVO> items, int offset) {
        if (items == null || items.isEmpty()) {
            return List.of();
        }
        int normalizedOffset = Math.floorMod(offset, items.size());
        List<HotTopicItemVO> rotated = new ArrayList<>(items.size());
        rotated.addAll(items.subList(normalizedOffset, items.size()));
        rotated.addAll(items.subList(0, normalizedOffset));
        return rotated;
    }

    private int refreshBatchShift(int itemCount) {
        return Math.min(REFRESH_BATCH_SHIFT, Math.max(1, itemCount));
    }

    private boolean sameTopics(List<HotTopicItemVO> left, List<HotTopicItemVO> right) {
        if (left == null || right == null || left.size() != right.size()) {
            return false;
        }
        return topicTitles(left).equals(topicTitles(right));
    }

    private Set<String> topicTitles(List<HotTopicItemVO> items) {
        Set<String> titles = new HashSet<>();
        for (HotTopicItemVO item : items) {
