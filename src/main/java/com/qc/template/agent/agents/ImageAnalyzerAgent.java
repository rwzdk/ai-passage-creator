...
public class ImageAnalyzerAgent implements NodeAction {
    ...
    public Map<String, Object> apply(OverAllState state) throws Exception {
        ...
        StreamHandlerContext.send(state.value("taskId").map(Object::toString).orElse(null), SseMessageTypeEnum.AGENT4_START.getValue());
        ...
        StreamHandlerContext.send(state.value("taskId").map(Object::toString).orElse(null),
                SseMessageTypeEnum.AGENT4_COMPLETE.getStreamingPrefix() + GsonUtils.toJson(validatedRequirements));
    ...
    private String getAllMethodsDescription() {
        return """
               ...
               - NANO_BANANA_APICLAUDE: 通过 apiclaude 接入 Nano Banana，适合创意插画、信息图表、抽象概念等 AI 生图
               - IMAGE_2: 适合通用 AI 生图、产品视觉和需要详细英文提示词描述的图片
    ...
    private String getMethodUsageDescription(ImageMethodEnum method) {
        return switch (method) {
            ...
            case IMAGE_2 -> "适合通用 AI 生图、产品视觉和需要详细英文提示词描述的图片";
